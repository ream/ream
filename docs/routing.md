...
In Ream, pages are Vue Components exported from `.js`, `.ts` or `.vue` files in the `src/routes` directory. Each page is associated with a route based on its file name.

**Example**: If you create `src/routes/about.vue` that exports a Vue component like below, it will be accessible at `/about`.
...
Consider the following page `src/routes/post/[pid].vue`:
...
Any route like `/post/1`, `/post/abc`, etc. will be matched by `src/routes/post/[pid].vue`. The matched value will be sent as a route parameter to the page.
...
Multiple dynamic route segments work the same way. The page `src/pages/post/[pid]/[comment].vue` will match the route `/post/abc/a-comment` and its parameters will be:
