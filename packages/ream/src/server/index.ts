...
const defaultApiHandler: ExtendServer = async (server, { apiRoutes }) => {
...
const defaultAppHandler: ExtendServer = async (
  server,
  { htmlTemplatePath, transformIndexHTML }
) => {
  debug.request('called default app handler')

  let html = await fs.promises.readFile(htmlTemplatePath, 'utf8')
...
export async function createHandler(options: CreateServerOptions) {
  ...
  const context = {
    apiRoutes: serverExports.apiRoutes,
    htmlTemplatePath,
